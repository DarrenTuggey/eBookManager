...
namespace eBookManager
{
    public partial class ImportBooks : Form
    {
        ...
        public ImportBooks()
        {
            ...
            this.txtPrice.PlaceholderText = $"{0:C2}";
            //DateTime date1 = new DateTime();
            //this.dtDatePublished.Format = (DateTimePickerFormat)1;
        }
        ...
        private void btnSaveNewStorageSpace_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtNewStorageSpaceName.Text.Length != 0)
                {
                    ...
                    bool spaceExists =
                    (!_spaces.StorageSpaceExists(newName, out int nextID)) ? false : throw new Exception("The storage space you are trying to add already exists.");
                    if (!spaceExists)
                    {
        ...
        private async Task UpdateStorageSpaceBooks(int storageSpaceId)
        {
            try
            {
                ...
                if (iCount > 0) // The space will always exist
                {
                    ...
                    if (iBooksExist > 0)
                    {
                        DialogResult dlgResult = MessageBox.Show($"A book with the same name has been found" +
                            "in Storage Space { existingSpace.Name}. Do you want to replace the existing book entry with this one ? ",
